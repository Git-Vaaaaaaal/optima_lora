...
# Logging setup
log_file = "virchow_lora_training.log"
logging.basicConfig(
    level=logging.INFO,
...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        ...
        factor=0.5,           # Reduce LR by half (less drastic than 0.1)
    ...
    best_val_acc = 0.0
    best_model_path = f'virchow_lora_fold_{fold + 1}_best.pth'
    epochs_no_improve = 0
    ...
    for epoch in range(num_epochs):
        ...
        for images_batch, labels_batch in tqdm(train_loader, desc=f"Fold {fold + 1} - Epoch {epoch + 1} Training"):
            ...
            if outputs.ndim == 3:  # [B, N, D] format 
                # Use CLS token (first token) for classification
                outputs = outputs[:, 0]  # Shape: [B, D]
            if outputs.ndim == 2 and outputs.size(1) > 1:  # [B, D] where D > 1
                # If output has multiple dimensions, apply a final linear layer or take mean
                if not hasattr(model, 'final_proj'):
                    model.final_proj = nn.Linear(outputs.size(1), 1).to(device)
                outputs = model.final_proj(outputs)
            
            # Ensure outputs are [B, 1] shape to match labels
            if outputs.ndim == 1:
                outputs = outputs.unsqueeze(1)  # [B] -> [B, 1]
            elif outputs.ndim == 2 and outputs.size(1) != 1:
                outputs = outputs[:, :1]  # Take first column if multiple
        ...
        with torch.no_grad():
            for images_batch, labels_batch in tqdm(val_loader, desc=f"Fold {fold + 1} - Epoch {epoch + 1} Validation"):
                ...
                # Handle different output formats (same as training)
                if outputs.ndim == 3:
                    outputs = outputs[:, 0]
