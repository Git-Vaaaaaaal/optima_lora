...
# Logging setup
log_file = "uni2h_lora_training.log"
logging.basicConfig(
    level=logging.INFO,
...
timm_kwargs = {
    ...
    'no_embed_class': True,
    'mlp_layer': timm.layers.SwiGLUPacked,  # Compatible with SwiGLU
...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    # Create transforms based on model config
    train_transform = create_transform(**resolve_data_config(model.pretrained_cfg, model=model))
    val_transform = create_transform(**resolve_data_config(model.pretrained_cfg, model=model))
    ...
    best_val_acc = 0.0
    best_model_path = f'uni2h_lora_fold_{fold + 1}_best.pth'
    epochs_no_improve = 0
