...
# Logging setup
log_file = "virchow2_lora_training.log"
logging.basicConfig(
    level=logging.INFO,
...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    best_val_acc = 0.0
    best_model_path = f'virchow2_lora_fold_{fold + 1}_best.pth'
    epochs_no_improve = 0
    ...
    for epoch in range(num_epochs):
        ...
        for images_batch, labels_batch in tqdm(train_loader, desc=f"Fold {fold + 1} - Epoch {epoch + 1} Training"):
            ...
            if outputs.ndim == 3:  # [B, N, D] format (Vision Transformer)
                # Use CLS token (first token) for classification
                outputs = outputs[:, 0]  # Use CLS token
