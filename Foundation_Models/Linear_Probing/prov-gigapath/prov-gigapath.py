...
# Logging setup
log_file = "gigapath_linear_probe_training.log"
logging.basicConfig(
    level=logging.INFO,
...
gigapath_model = timm.create_model(model_name, pretrained=True)
gigapath_model.eval().to(device)

# GigaPath transform
gigapath_transform = transforms.Compose([
...
# Classifier head
class GigaPathLinearClassifier(nn.Module):
    def __init__(self):
        super(GigaPathLinearClassifier, self).__init__()
...
class EmbeddingDataset(Dataset):
    ...
    def _extract_embeddings(self):
        ...
        for img_path in tqdm(self.image_paths, desc="Extracting embeddings"):
            ...
            image_tensor = gigapath_transform(image).unsqueeze(0).to(device)
            with torch.no_grad():
                output = gigapath_model(image_tensor)
...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    model = GigaPathLinearClassifier().to(device)
    ...
    best_val_acc = 0.0
    best_model_path = f'gigapath_linear_probe_fold_{fold + 1}_best.pth'
    epochs_no_improve = 0
