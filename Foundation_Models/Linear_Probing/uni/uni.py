...
# Logging setup
log_file = "uni_linear_probe_training.log"
logging.basicConfig(
    level=logging.INFO,
...
uni_transform = create_transform(**uni_config)

# Classifier head
class UNILinearClassifier(nn.Module):
    def __init__(self):
        super(UNILinearClassifier, self).__init__()
...
class EmbeddingDataset(Dataset):
    ...
    def _extract_embeddings(self):
        ...
        for img_path in tqdm(self.image_paths, desc="Extracting embeddings"):
            ...
            image_tensor = uni_transform(image).unsqueeze(0).to(device)
            with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.float16):
                output = uni_model(image_tensor)
...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    model = UNILinearClassifier().to(device)
    ...
    best_val_acc = 0.0
    best_model_path = f'uni_linear_probe_fold_{fold + 1}_best.pth'
    epochs_no_improve = 0
