...
# Logging setup
log_file = "uni2h_linear_probe_training.log"
logging.basicConfig(
    level=logging.INFO,
...
uni2h_model = timm.create_model(model_name, pretrained=True, **timm_kwargs)
uni2h_model.eval().to(device)
uni2h_config = resolve_data_config(uni2h_model.pretrained_cfg, model=uni2h_model)
uni2h_transform = create_transform(**uni2h_config)

# Classifier head
class UNI2hLinearClassifier(nn.Module):
    def __init__(self):
        super(UNI2hLinearClassifier, self).__init__()
...
class EmbeddingDataset(Dataset):
    ...
    def _extract_embeddings(self):
        ...
        for img_path in tqdm(self.image_paths, desc="Extracting embeddings"):
            ...
            image_tensor = uni2h_transform(image).unsqueeze(0).to(device)
            with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.float16):
                output = uni2h_model(image_tensor)
                embedding = output.squeeze(0).to(torch.float32)
...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    model = UNI2hLinearClassifier().to(device)
    ...
    best_val_acc = 0.0
    best_model_path = f'uni2h_linear_probe_fold_{fold + 1}_best.pth'
    epochs_no_improve = 0
