...
# Logging setup
log_file = "hoptimus1_linear_probe.log"
logging.basicConfig(
    level=logging.INFO,
...
# Transform - same normalization as H-optimus-0
...
class HoptimusBinaryClassifier(nn.Module):
    def __init__(self):
        super().__init__()
        self.classifier = nn.Linear(1536, 1)  # No hidden layer, no dropout
...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    best_model_path = f'hoptimus1_linear_probe_fold_{fold + 1}_best.pth'
