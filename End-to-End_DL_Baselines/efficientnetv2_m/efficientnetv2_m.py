...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    best_model_path = f'amibr_efficientnetv2m_fold_{fold + 1}_best_full_model.pth'
    epochs_no_improve = 0
    ...
    for epoch in range(num_epochs):
        ...
        if val_balanced_accuracy > best_val_balanced_accuracy:
            ...
            # Save the full model instead of just state_dict
...
# Example of how to load the saved full model for inference
"""
To load the saved full model for inference:

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = torch.load('amibr_efficientnetv2m_fold_1_best_full_model.pth', map_location=device)
model.eval()
