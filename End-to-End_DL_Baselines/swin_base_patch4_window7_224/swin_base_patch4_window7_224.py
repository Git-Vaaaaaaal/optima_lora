...
# Logging setup
log_file = "amibr_swin_base_patch4_window7_224.log"
logging.basicConfig(
    level=logging.INFO,
...
for fold, (train_idx, val_idx) in enumerate(strat_group_kfold.split(images, labels, groups)):
    ...
    best_model_path = f'amibr_swin_base_patch4_window7_224_fold_{fold + 1}_best.pth'
    epochs_no_improve = 0
    ...
    for epoch in range(num_epochs):
        ...
        train_preds = (torch.sigmoid(torch.tensor(np.array(train_preds_logits))) > 0.5).numpy().astype(int)
        ...
        val_preds = (torch.sigmoid(torch.tensor(np.array(val_preds_logits))) > 0.5).numpy().astype(int)
